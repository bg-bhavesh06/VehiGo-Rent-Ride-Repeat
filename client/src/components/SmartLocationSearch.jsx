...
const SmartLocationSearch = ({
  ...
  const showNotification = (type, message) => {
    ...
    setTimeout(() => {
      setNotification((prev) =>
        prev && prev.message === message ? null : prev,
      );
    }, 5000);
  };
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        const cleanedCities = [
          ...new Set(data.map((loc) => loc.split(",")[0].trim())),
        ].filter(Boolean);
  ...
  return (
    <div
      className={`relative w-full flex ${isSidebar ? "flex-col gap-2" : "flex-col md:flex-row gap-4"}`}
      ref={dropdownRef}
    >
      <div className="flex-1 relative">
        <MapPin
          className={`absolute top-1/2 transform -translate-y-1/2 text-gray-400 ${isSidebar ? "left-2.5 h-4 w-4" : "left-3 h-5 w-5"}`}
        />
        <input
          type="text"
          value={query}
        ...
        {loading && (
          <div
            className={`absolute top-1/2 transform -translate-y-1/2 ${isSidebar ? "right-3" : "right-24"}`}
          >
            <div
              className={`animate-spin rounded-full border-b-2 border-primary-600 ${isSidebar ? "h-3.5 w-3.5" : "h-4 w-4"}`}
            ></div>
          </div>
        )}
